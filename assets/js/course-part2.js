...
let courseStarted = CourseState.isPartStarted(2);
...
if ((isAdminUser() || CourseState.isPart1Completed()) && courseStarted) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!isAdminUser() && !CourseState.isPart1Completed()) {
...
function initializeCourse() {
  ...
  if (startBtn) {
    startBtn.addEventListener("click", () => {
      ...
      if (jsEditor) jsEditor.updateOptions({ readOnly: false });
...
require(["vs/editor/editor.main"], function () {
  if (!isAdminUser() && !CourseState.isPart1Completed()) return;
  ...
  // Live preview update
  htmlEditor.onDidChangeModelContent(debounce(updatePreview, 100));
  cssEditor.onDidChangeModelContent(debounce(updatePreview, 100));
  jsEditor.onDidChangeModelContent(debounce(updatePreview, 100));
